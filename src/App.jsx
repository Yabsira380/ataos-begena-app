...
const EthiopianCross = ({ className = "w-6 h-6" }) => (
  <svg viewBox="0 0 100 100" className={className} fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M 35 35 L 45 25 L 55 25 L 65 35 L 55 45 L 45 45 Z" opacity="0.3" fill="none" stroke="currentColor" strokeWidth="2"/>
...
export default function App() {
  ...
  const handleAddStudentSubmit = (e) => {
    ...
    triggerConfirmation(
      `አዲስ ተማሪ "${newStudent.name}" ለመመዝገብ መረጃው ትክክል መሆኑን ያረጋግጣሉ?`,
      'የተማሪ ምዝገባ ማረጋገጫ',
      async () => {
        ...
        const studentToInsert = {
          ...
          christian_name: newStudent.christianName,
          phone: newStudent.phone,
          emergency_contact_name: newStudent.emergencyContactName,
          emergency_contact_phone: newStudent.emergencyContactPhone,
          work_status: newStudent.workStatus,
          church_service: newStudent.churchService,
          parish: newStudent.parish,
          instrument_type: newStudent.instrumentType,
          duration: newStudent.duration,
          chosen_day: newStudent.chosenDay,
          chosen_time: newStudent.chosenTime,
          payment_amount: Number(newStudent.paymentAmount || 0),
        ...
        if (error) {
          showNotification('ምዝገባው አልተሳካም! እባክዎ እንደገና ይሞክሩ።', 'error');
        } else {
  ...
  const askToDeleteStudent = (student) => {
    triggerConfirmation(
      ...
      async () => {
        ...
        if (error) {
          showNotification('ስረዛው አልተሳካም!', 'error');
        } else {
  ...
  const handleExamScoreSubmit = (studentId, score) => {
    ...
    if (!student) return;
    triggerConfirmation(
      `የተማሪ "${student.name}" የፈተና ውጤት ወደ ${score}% እንዲቀየር ይፈልጋሉ?`,
      'የፈተና ውጤት ማረጋገጫ',
      async () => {
      ...
      <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-fade-in">
        <div className="bg-[#FAF3E0] rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border-4 border-[#8B5A2B] relative flex flex-col">
          ...
          <div className="p-6 text-center space-y-4">
            ...
            <p className="text-sm font-medium text-[#4A3219] leading-relaxed py-2 px-1">
              {confirmModal.message}
            </p>
            <div className="flex gap-3 pt-2">
              <button onClick={() => setConfirmModal(prev => ({ ...prev, show: false }))} className="flex-1 py-3 bg-white hover:bg-gray-100 text-[#8B5A2B] border-2 border-[#D2B48C] font-bold rounded-xl transition-all active:scale-95 text-xs shadow-sm">
                እመለሳለሁ (Cancel)
              </button>
              <button onClick={confirmModal.onConfirm} className="flex-1 py-3 bg-[#8B5A2B] hover:bg-[#5C4033] text-white font-extrabold rounded-xl transition-all active:scale-95 text-xs shadow-md border border-[#D4AF37]">
                አረጋግጣለሁ (Confirm)
              </button>
            </div>
          </div>
             ...
             <div className="flex items-center space-x-4">
                <div className="relative">
                  {updatedStudentObj.photo ? (
                    <img src={updatedStudentObj.photo} alt={updatedStudentObj.name} className="w-16 h-16 rounded-xl object-cover border-2 border-[#D4AF37]" />
                  ) : (
                ...
                <div>
                  ...
                  <p className="text-xs text-[#D4AF37] font-bold mt-0.5"> ክ.ስ: {updatedStudentObj.christianName || '-'}</p>
                  <p className="text-[10px] bg-black/40 inline-block px-2 py-0.5 rounded mt-1"> መ.ቁ: #{updatedStudentObj.studentNo}</p>
                </div>
             </div>
             <button onClick={() => setSelectedStudentProfile(null)} className="p-1.5 bg-[#FAF3E0]/10 hover:bg-white/20 rounded-full text-[#D4AF37]"><X size={20} /></button>
          </div>
          ...
          <div className="p-5 space-y-4">
             <div className={`p-3 rounded-xl border flex items-center justify-between shadow-sm ${updatedStudentObj.status === 'completed' ? 'bg-[#E8F5E9] border-[#A5D6A7] text-[#2E7D32]' : updatedStudentObj.status === 'dropped' ? 'bg-[#FFEBEE] border-[#EF9A9A] text-[#C62828]' : 'bg-[#E3F2FD] border-[#90CAF9] text-[#1565C0]'}`}>
               <span className="font-bold text-xs flex items-center">
                 {updatedStudentObj.status === 'completed' ? <><Award size={18} className="mr-2"/> ትምህርቱን ያጠናቀቀ </> : 
                  updatedStudentObj.status === 'dropped' ? <><UserMinus size={18} className="mr-2"/> ትምህርቱን ያቋረጠ </> : 
                  <><BookOpen size={18} className="mr-2"/> በመማር ላይ ያለ (Active)</>}
               </span>
               {updatedStudentObj.examResult && <span className="text-xs font-black bg-white px-2 py-1 rounded shadow-sm text-gray-700"> ውጤት: {updatedStudentObj.examResult}%</span>}
             </div>
        ...
        <div className="sticky top-0 bg-[#3E2723] text-white p-4 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 shadow-md hide-on-print z-50 border-b-4 border-[#D4AF37]">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="bg-[#FAF3E0]/15 p-1 rounded-lg flex gap-1 border border-white/20">
              ...
              <button onClick={() => setReportConfig(prev => ({...prev, statusFilter: 'active'}))} className={`px-2 py-1 rounded text-[10px] font-black transition-colors ${reportConfig.statusFilter === 'active' ? 'bg-blue-700 text-white' : 'text-gray-300 hover:text-white'}`}>በመማር ላይ (Active)</button>
              <button onClick={() => setReportConfig(prev => ({...prev, statusFilter: 'dropped'}))} className={`px-2 py-1 rounded text-[10px] font-black transition-colors ${reportConfig.statusFilter === 'dropped' ? 'bg-red-700 text-white' : 'text-gray-300 hover:text-white'}`}>የቆረጡ (Dropped)</button>
      ...
      <div className="p-5 space-y-6 animate-fade-in pb-12 relative z-10">
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex flex-col items-end space-y-2">
            <button onClick={() => setReportConfig({show: true, type: 'general', statusFilter: 'all'})} className="bg-[#8B5A2B] hover:bg-[#5C4033] text-white p-2.5 rounded-xl shadow-md transition-all active:scale-95 flex items-center space-x-1 border border-[#D4AF37]" title="ሪፖርት አውጣ (PDF)">
              <Printer size={18} /><span className="text-xs font-bold px-1 hidden sm:inline"> አትም </span>
            </button>
            <div className="flex space-x-1">
               <div className="relative">
                  <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="appearance-none bg-white border border-[#C19A6B] text-[#5C4033] text-xs font-bold py-2 pl-2 pr-6 rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-[#8B5A2B]">
                    {ethiopianYears.map(y => <option key={y} value={y}>{y}</option>)}
                  </select>
                  <ChevronDown className="absolute right-1 top-2.5 text-[#8B5A2B] pointer-events-none" size={12} />
               </div>
               <div className="relative">
                  <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="appearance-none bg-[#F5E6D3] border border-[#C19A6B] text-[#5C4033] text-xs font-bold py-2 pl-2 pr-6 rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-[#8B5A2B]">
      ...
      <form onSubmit={handleAddStudentSubmit} className="space-y-5">
        <div className="bg-white/90 backdrop-blur-sm p-5 rounded-3xl shadow-sm border-2 border-[#EADDCA] flex flex-col items-center justify-center">
          <label className="relative cursor-pointer w-24 h-24 bg-[#F5E6D3] rounded-2xl flex items-center justify-center overflow-hidden border-2 border-dashed border-[#C19A6B] hover:border-[#8B5A2B] transition-all mb-2">
            {newStudent.photo ? <img src={newStudent.photo} alt="Student" className="w-full h-full object-cover" /> : <div className="flex flex-col items-center text-[#8B5A2B]"><Camera size={24} className="mb-1" /><span className="text-[10px] font-bold"> ፎቶ ያክሉ </span></div>}
            <input type="file" accept="image/*" onChange={handlePhotoUpload} className="hidden" />
          </label>
        </div>
        <div className="bg-[#FAF3E0]/95 backdrop-blur-sm p-5 rounded-3xl shadow-sm border-2 border-[#D2B48C]">
          <h3 className="font-extrabold text-[#3E2723] border-b-2 border-dashed border-[#D2B48C] pb-2 mb-4 text-sm flex items-center"><User size={16} className="mr-2 text-[#8B5A2B]"/> የተመዝጋቢው የግል መረጃ </h3>
    ...
    return (
      <div className="p-5 animate-fade-in pb-12 relative z-10">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-black text-[#2E1A05] font-serif leading-tight"> የተማሪዎች ሁኔታ መዝገብ <br/><span className="text-xs text-[#8B5A2B] font-bold"> የፈተና ውጤት እና ድርጊቶች </span></h2>
        ...
        <div className="flex p-1 bg-[#F5E6D3]/90 backdrop-blur-sm rounded-xl mb-5 border-2 border-[#D2B48C] overflow-x-auto">
          <button onClick={() => setAcademicViewType('active')} className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-colors flex items-center justify-center min-w-[90px] ${academicViewType === 'active' ? 'bg-[#8B5A2B] text-white shadow-sm' : 'text-[#8B5A2B]/80 hover:bg-white/50'}`}>
             <BookOpen size={14} className="mr-1"/> በመማር ላይ
          </button>
          <button onClick={() => setAcademicViewType('completed')} className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-colors flex items-center justify-center min-w-[90px] ${academicViewType === 'completed' ? 'bg-green-700 text-white shadow-sm' : 'text-[#8B5A2B]/80 hover:bg-white/50'}`}>
             <Award size={14} className="mr-1"/> ያጠናቀቁ
          </button>
          <button onClick={() => setAcademicViewType('dropped')} className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-colors flex items-center justify-center min-w-[90px] ${academicViewType === 'dropped' ? 'bg-red-700 text-white shadow-sm' : 'text-[#8B5A2B]/80 hover:bg-white/50'}`}>
        ...
        <div className="space-y-4">
          {filteredInfoStudents.map(student => (
            <div key={student.id} className="p-4 bg-[#FAF8F2]/95 backdrop-blur-sm rounded-3xl shadow-sm border-2 border-[#EADDCA] relative overflow-hidden">
              <div className="absolute -top-6 -right-6 opacity-[0.04] pointer-events-none"><EthiopianCross className="w-24 h-24 text-[#8B5A2B]" /></div>
              <div onClick={() => setSelectedStudentProfile(student)} className="flex items-center space-x-3 cursor-pointer mb-3 pb-3 border-b border-gray-200 relative z-10">
                <div className="relative">
                  {student.photo ? <img src={student.photo} alt={student.name} className="w-12 h-12 rounded-full object-cover border-2 border-[#D2B48C]" /> : <div className="w-12 h-12 bg-[#F5E6D3] rounded-full flex items-center justify-center border-2 border-[#D2B48C]"><User size={20} className="text-[#8B5A2B]" /></div>}
                </div>
                <div className="flex-1">
                  <h3 className="font-extrabold text-[#2E1A05] text-sm">{student.name} <span className="text-[10px] text-gray-500 font-normal">#{student.studentNo}</span></h3>
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    <span className="text-[9px] text-[#8B5A2B] font-bold bg-[#F5E6D3] px-2 py-0.5 rounded">{student.instrumentType}</span>
                    <span className="text-[9px] text-gray-600 bg-gray-100 px-2 py-0.5 rounded font-mono">መዝገብ፦ {student.registrationDate || '-'}</span>
                  </div>
                </div>
                <button onClick={(e) => { e.stopPropagation(); askToDeleteStudent(student); }} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
              </div>
                 ...
                 <div className="flex-[1.5] flex flex-col justify-end">
                    ...
                    {student.status === 'active' ? (
                       <div className="flex gap-1.5">
                         <button onClick={() => setStudentStatusWithConfirm(student, 'completed')} className="flex-1 bg-[#E8F5E9] hover:bg-green-100 text-[#2E7D32] border border-[#A5D6A7] py-2 rounded-lg text-[10px] sm:text-xs font-extrabold transition-colors flex items-center justify-center">
                           <GraduationCap size={14} className="mr-1 hidden sm:block"/> ምረቅ
                         </button>
                         <button onClick={() => setStudentStatusWithConfirm(student, 'dropped')} className="flex-1 bg-[#FFEBEE] hover:bg-red-100 text-[#C62828] border border-[#EF9A9A] py-2 rounded-lg text-[10px] sm:text-xs font-extrabold transition-colors flex items-center justify-center">
                           <UserMinus size={14} className="mr-1 hidden sm:block"/> አቋርጧል
                         </button>
                       </div>
                    ) : (
                       <button onClick={() => setStudentStatusWithConfirm(student, 'active')} className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-300 py-2 rounded-lg text-xs font-bold transition-colors flex items-center justify-center">
          ...
          {filteredInfoStudents.length === 0 && <p className="text-center text-[#8B5A2B] text-sm py-8 font-bold"> በዚህ ክፍል የተመዘገበ ተማሪ የለም። </p>}
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="p-5 animate-fade-in pb-12 relative z-10">
        ...
        <div className="grid grid-cols-3 gap-2 bg-[#F5E6D3]/90 backdrop-blur-sm p-2.5 rounded-2xl shadow-sm border-2 border-[#D2B48C] mb-5">
          <div><span className="text-[10px] font-black text-[#5C4033] block mb-1"> ዓመተ ምሕረት፦ </span><select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="w-full bg-white border border-[#D2B48C] text-[#5C4033] font-bold py-2 px-1 rounded-xl text-xs focus:ring-1 focus:ring-[#8B5A2B]">{ethiopianYears.map(y => <option key={y} value={y}>{y}</option>)}</select></div>
          <div><span className="text-[10px] font-black text-[#5C4033] block mb-1"> የዕለት ወር፦ </span><select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="w-full bg-white border border-[#D2B48C] text-[#5C4033] font-bold py-2 px-1 rounded-xl text-xs focus:ring-1 focus:ring-[#8B5A2B]">{ethiopianMonths.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
          <div><span className="text-[10px] font-black text-[#5C4033] block mb-1"> ዕለት ቀን፦ </span><select value={selectedDay} onChange={(e) => setSelectedDay(Number(e.target.value))} className="w-full bg-white border border-[#D2B48C] text-[#5C4033] font-bold py-2 px-1 rounded-xl text-xs focus:ring-1 focus:ring-[#8B5A2B]">{Array.from({ length: 30 }, (_, i) => i + 1).map(day => <option key={day} value={day}>{day}</option>)}</select></div>
        </div>
        <div className="relative mb-5"><Search size={18} className="absolute left-3 top-3.5 text-[#8B5A2B]/60" /><input type="text" placeholder="በስም ወይም በመለያ ቁጥር ይፈልጉ..." value={attendanceSearch} onChange={(e) => setAttendanceSearch(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white/95 border-2 border-[#EADDCA] rounded-xl text-sm font-bold shadow-sm" /></div>
            ...
            return (
              <div key={student.id} className="flex items-center justify-between p-4 bg-white/95 backdrop-blur-sm rounded-3xl shadow-sm border-2 border-[#EADDCA]">
                <div onClick={() => setSelectedStudentProfile(student)} className="flex items-center space-x-4 cursor-pointer">
                  <div className="relative"><div className="w-12 h-12 bg-[#F5E6D3] rounded-2xl flex items-center justify-center border-2 border-[#D2B48C] overflow-hidden">{student.photo ? <img src={student.photo} alt="" className="w-full h-full object-cover"/> : <User size={20} className="text-[#8B5A2B]"/>}</div><div className="absolute -bottom-1 -right-2 bg-[#2E1A05] border border-[#D4AF37] text-[#FAF3E0] text-[9px] font-black px-1.5 py-0.5 rounded shadow">#{student.studentNo}</div></div>
                  <div><h3 className="font-extrabold text-[#2E1A05] text-sm">{student.name}</h3><p className="text-[10px] text-gray-500 font-bold">{student.chosenDay} | {student.chosenTime}</p></div>
                </div>
                <button 
  ...
  const renderPaymentsView = () => {
    ...
    return (
      <div className="p-5 animate-fade-in pb-12 relative z-10">
        ...
        <div className="relative mb-5"><Search size={18} className="absolute left-3 top-3.5 text-[#8B5A2B]/60" /><input type="text" placeholder="በስም ወይም በመለያ ቁጥር ይፈልጉ..." value={paymentSearch} onChange={(e) => setPaymentSearch(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white/95 border-2 border-[#EADDCA] rounded-xl text-sm font-bold shadow-sm" /></div>
  ...
  const renderAiAssistantView = () => (
    <div className="p-5 animate-fade-in h-full flex flex-col pb-12 relative z-10">
      <div className="flex items-center space-x-3 mb-6">
        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#D4AF37] to-[#8B5A2B] flex items-center justify-center shadow-lg border-2 border-[#FAF3E0]"><Sparkles className="text-white" size={24} /></div>
        <div>
      ...
      <div className="flex-1 bg-white/90 backdrop-blur-md rounded-3xl border-2 border-[#EADDCA] shadow-inner p-4 mb-4 overflow-y-auto min-h-[300px] flex flex-col relative">
        ...
        {aiResponse && <div className="bg-[#5C4033] text-[#F5E6D3] p-4 rounded-2xl rounded-tr-none mb-4 max-w-[90%] self-end text-sm shadow-md whitespace-pre-wrap leading-relaxed font-semibold border border-[#D4AF37]">{aiResponse}</div>}
        {isAiLoading && <div className="flex items-center space-x-2 text-[#8B5A2B] self-end mb-4"><Loader2 className="animate-spin" size={20} /><span className="text-sm font-bold"> ምላሽ በማዘጋጀት ላይ...</span></div>}
        <div ref={chatEndRef} />
      </div>
      <form onSubmit={askAI} className="relative mt-auto">
        ...
        <button type="submit" disabled={isAiLoading || !aiQuery.trim()} className="absolute right-2 top-2 bottom-2 w-10 h-10 bg-[#8B5A2B] text-white rounded-full flex items-center justify-center hover:bg-[#5C4033] disabled:opacity-50 shadow-md"><Send size={18} className="ml-1" /></button>
      </form>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="h-screen bg-[#FAF4E8] flex flex-col items-center justify-center">
        <Loader2 className="animate-spin text-[#8B5A2B] w-12 h-12 mb-3" />
        <p className="text-[#8B5A2B] font-extrabold text-sm font-serif">የተማሪዎች መረጃ ከኦንላይን ዳታቤዝ በመጫን ላይ ነው...</p>
      </div>
    );
  }
      ...
      <div className="app-ui h-screen bg-[#FAF4E8] font-sans max-w-md mx-auto relative shadow-2xl overflow-hidden flex flex-col selection:bg-[#EADDCA]">
        ...
        <header className="bg-gradient-to-r from-[#2E1A05] via-[#4A2E12] to-[#2E1A05] py-4 px-6 sticky top-0 z-20 shadow-xl border-b-4 border-[#D4AF37]">
          <div className="flex items-center justify-center space-x-3">
            <div className="text-[#D4AF37]"><BegenaIcon className="w-8 h-8 drop-shadow-md" /></div>
        ...
        {notification.show && (
          <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-[150] w-11/12 max-w-sm animate-fade-in hide-on-print">
            <div className={`p-3 rounded-2xl shadow-xl flex items-center space-x-3 border-2 ${notification.type === 'success' ? 'bg-[#E8F5E9] border-green-500 text-[#1B5E20]' : 'bg-[#FFEBEE] border-[#F44336] text-[#B71C1C]'}`}>
              <Info size={20} /><span className="font-extrabold text-xs">{notification.message}</span>
            </div>
          </div>
        )}
        ...
        <nav className="absolute bottom-4 left-3 right-3 bg-white/95 backdrop-blur-md border-2 border-[#D2B48C] rounded-full shadow-[0_8px_30px_rgba(139,90,43,0.3)] z-50 hide-on-print">
          <div className="flex justify-between items-center p-1.5 px-2 sm:px-4">
            {[
              ...
              <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center justify-center w-11 h-11 sm:w-12 sm:h-12 rounded-full transition-all duration-300 relative ${activeTab === item.id ? item.special ? 'bg-gradient-to-b from-[#D4AF37] to-[#8B5A2B] text-white shadow-lg border-2 border-[#FAF3E0] transform -translate-y-2' : 'text-[#8B5A2B]' : 'text-[#BCAAA4] hover:text-[#8D6E63]'}`}>
                {activeTab === item.id && !item.special && <span className="absolute inset-0 bg-[#FAF3E0] border border-[#D2B48C] rounded-full scale-110 -z-10" />}
                <item.icon size={item.special ? 20 : 18} />
                {activeTab === item.id && !item.special && <span className="text-[8px] sm:text-[9px] font-black mt-0.5 absolute -bottom-3 text-[#5C4033]">{item.label}</span>}
              </button>
            ))}
          </div>
        </nav>
